...
- **管理者は `admin.html` で、全受講生の進捗を一覧で確認できる**
...
- `admin.html` はサイト内のどこからもリンクしていない(受講生用ページに管理者リンクは出てこない)。URLを知っている人だけがアクセスできる、というシンプルな仕組み
...
[`config.js`](config.js) を開いて、コピーしたURLを貼り付けます。
...
[http://localhost:8090](http://localhost:8090) で受講生用ページ、[http://localhost:8090/admin.html](http://localhost:8090/admin.html) で管理者一覧を確認できます。
...
`admin.html` を開くと、達成日数の多い順に全受講生が並びます。受講生用のページからはリンクしていないので、このURLは管理者だけが直接開いてください。

例: `https://(あなたのアカウント).github.io/(リポジトリ名)/admin.html`
